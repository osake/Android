...
public class BubbleActivity extends Activity {
	...
	private void setupGestureDetector() {
		...
		new GestureDetector.SimpleOnGestureListener() {
			...
			public boolean onSingleTapConfirmed(MotionEvent event) {
				...
				if (newBubble) {
					BubbleView bubbleView = new BubbleView(getApplicationContext(), event.getX(),
							event.getY());
	...
	private class BubbleView extends View {
		...
		private synchronized boolean intersects(float x, float y) {
			...
			return mXPos < x && x < mXPos + mScaledBitmap.getWidth() && mYPos < y
					&& y < mYPos + mScaledBitmap.getHeight();
		}
		...
		private boolean isOutOfView() {
			...
			return mXPos < 0 || mYPos < 0 || mXPos + mScaledBitmap.getWidth() > maxScreenSizeX
					|| mYPos + mScaledBitmap.getHeight() > maxScreenSizeY;
