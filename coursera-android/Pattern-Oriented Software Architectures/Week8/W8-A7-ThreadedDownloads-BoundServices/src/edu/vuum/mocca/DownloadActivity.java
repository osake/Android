 ...
 * image and download that image using one of two different Android Bound
 * Service implementations: synchronous and asynchronous. The Activity starts
 * the Service using bindService(). After the Service is started, its onBind()
 * hook method returns an implementation of an AIDL interface to the Activity by
 * asynchronously calling the onServiceConnected() hook method in the Activity.
 ...
 * the underlying Android Binder framework, shielding applications from tedious
 * and error-prone aspects of inter-process communication.
 */
public class DownloadActivity extends DownloadBase {
	 ...
	 * DownloadServiceSync Service. This object plays the role of Requestor in
	 * the Broker Pattern. If it's null then there's no connection to the
	 * Service.
	 */
	 ...
	 * DownloadServiceAsync Service. This plays the role of Requestor in the
	 * Broker Pattern. If it's null then there's no connection to the Service.
	 */
	...
	ServiceConnection mServiceConnectionSync = new ServiceConnection() {
		/**
		 * Cast the returned IBinder object to the DownloadCall AIDL Interface
		 * and store it for later use in mDownloadCall.
		 */
		@Override
		public void onServiceConnected(ComponentName name, IBinder service) {
			...
			// DownloadBoundServiceSync syncService =
			// (DownloadBoundServiceSync)service;
			mDownloadCall = DownloadCall.Stub.asInterface(service);
		}

		/**
		 * Called if the remote service crashes and is no longer available. The
	...
	ServiceConnection mServiceConnectionAsync = new ServiceConnection() {
		/**
		 * Cast the returned IBinder object to the DownloadRequest AIDL
		 * Interface and store it for later use in mDownloadRequest.
		 */
		@Override
		public void onServiceConnected(ComponentName name, IBinder service) {
			// TODO You fill in here to replace null with a call
			...
			// service parameter into an interface that can be
			// used to make RPC calls to the Service.
			// DownloadBoundServiceAsync asyncService =
			// (DownloadBoundServiceAsync)service;
	...
	DownloadCallback.Stub mDownloadCallback = new DownloadCallback.Stub() {
		/**
		 * Called when the DownloadServiceAsync finishes obtaining the results
		 * from the GeoNames Web service. Use the provided String to display the
		 * results in a TextView.
		 */
		@Override
	...
	/**
	 * Hook method called when the DownloadActivity becomes visible to bind the
	...
	public void onStart() {
		...
		if (mDownloadCall == null)
			bindService(DownloadBoundServiceSync.makeIntent(this), mServiceConnectionSync,
					BIND_AUTO_CREATE);
		if (mDownloadRequest == null)
			bindService(DownloadBoundServiceAsync.makeIntent(this), mServiceConnectionAsync,
					BIND_AUTO_CREATE);
	}

	/**
	 * Hook method called when the DownloadActivity becomes completely hidden to
